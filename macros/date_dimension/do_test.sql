...
{% set tbl = db ~ '.' ~ schema ~ '.test_date_dimension' %}
{% set tbl_fy = db ~ '.' ~ schema ~ '.test_date_dimension_fy' %}
{% set tbl_ph = db ~ '.' ~ schema ~ '.test_public_holidays' %}

{% do run_query("create database if not exists " ~ db) %}
{% do run_query("create schema if not exists " ~ db ~ "." ~ schema) %}

{% do run_query("
    create or replace table " ~ tbl_ph ~ " (
        date date not null,
...
{% do run_query("
    insert into " ~ tbl_ph ~ " values
        ('2024-01-01'::date, 'New Year''s Day', 'NL'),
        ('2024-03-31'::date, 'Easter Sunday', 'NL'),
        ('2024-04-01'::date, 'Easter Monday', 'NL'),
        ('2025-04-26'::date, 'King''s Day', 'NL'),
        ('2024-12-25'::date, 'Christmas Day', 'NL')
") %}

{% set ph_rel = adapter.get_relation(
    database=target.database,
    schema=schema,
    identifier='test_public_holidays'
) %}
